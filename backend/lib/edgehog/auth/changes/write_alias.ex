#
# This file is part of Edgehog.
#
# Copyright 2026 SECO Mind Srl
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# SPDX-License-Identifier: Apache-2.0
#

defmodule Edgehog.Auth.Changes.WriteAlias do
  @moduledoc """
  A change to create an alias for an object in the FGA provider.

  Opts:
  - alias_attr   :: atom. the attribute to be used as an alias for the resource
  - fga_type     :: atom. the type assigned to the resource in the FGA model

  An alias is useful, for example, for resources defining an attribute to use for their FGA id that is different than the
  their actual id: this way the FGA service can still check programmatically by using the id provided by Ash, but it is
  also possible for a human to inspect the status manually.

  For example, devices use `device_id` instead of `id`. With this change, the tuples
  ```fga
    {device:Simpl3D3vice1d, alias, device:<uuid>}
  ```
  and
  ```fga
    {device:<uuid>, alias, device:Simpl3D3vice1d}
  ```
  will be written by the FGA service.
  """
  use Ash.Resource.Change

  alias Edgehog.Auth.FGAService

  @needed_keys [:alias_attr, :fga_type]

  @impl Ash.Resource.Change
  def init(opts) do
    {_, errors} = Enum.reduce(@needed_keys, {opts, []}, &error_if_not_present/2)

    if Enum.empty?(errors),
      do: {:ok, opts},
      else: {:error, errors}
  end

  @impl Ash.Resource.Change
  def change(%{valid: false} = changeset, _opts, _context), do: changeset

  @impl Ash.Resource.Change
  def change(changeset, opts, _context) do
    Ash.Changeset.after_action(changeset, &write_alias(&1, &2, opts))
  end

  @impl Ash.Resource.Change
  def atomic(%{valid: false}, _opts, _context), do: :ok

  @impl Ash.Resource.Change
  def atomic(changeset, opts, _context) do
    Ash.Changeset.after_action(changeset, &write_alias(&1, &2, opts))

    :ok
  end

  defp write_alias(%{action_type: :create} = _changeset, record, opts),
    do: create_alias(record, opts)

  defp write_alias(%{action_type: :update} = changeset, record, opts),
    do: update_alias(changeset, record, opts)

  defp create_alias(record, opts) do
    alias_attr = Keyword.get(opts, :alias_attr)
    type = Keyword.get(opts, :fga_type)

    id = Map.get(record, :id)
    fga_id = "#{type}:#{id}"
    alias = Map.get(record, alias_attr)
    fga_alias = "#{type}:#{alias}"

    if type == "device" do
      dbg({fga_alias, "alias", fga_id})
    end

    FGAService.write(fga_alias, "alias", fga_id)
    FGAService.write(fga_id, "alias", fga_alias)

    {:ok, record}
  end

  defp update_alias(changeset, record, opts) do
    alias_attr = Keyword.get(opts, :alias_attr)
    type = Keyword.get(opts, :fga_type)
    old_alias = Ash.Changeset.fetch_argument(changeset, alias_attr)

    case old_alias do
      # The update didn't change the alias
      :error ->
        {:ok, record}

      {:ok, old_alias} ->
        id = Map.get(record, :id)
        fga_id = "#{type}:#{id}"

        alias = Map.get(record, alias_attr)
        fga_alias = "#{type}:#{alias}"
        old_fga_alias = "#{type}:#{old_alias}"

        if fga_alias != old_fga_alias do
          FGAService.delete(old_fga_alias, "alias", fga_id)
          FGAService.delete(fga_id, "alias", old_fga_alias)

          FGAService.write(fga_alias, "alias", fga_id)
          FGAService.write(fga_id, "alias", fga_alias)
        end

        {:ok, record}
    end
  end

  defp error_if_not_present(key, {opts, errors}) do
    error = "Missing key #{inspect(key)}."

    if Keyword.has_key?(opts, key),
      do: {opts, errors},
      else: {opts, [error | errors]}
  end
end
